...
void TextFrame::OnMenuFileOpen(wxCommandEvent &event) {
	...
	if (OpenDialog->ShowModal() == wxID_OK) {
		m_pTextCtrl->LoadFile(OpenDialog->GetPath()) ? SetStatusText(_T("Loaded")) : SetStatusText(_T("Load Failed"));
	}
...
void TextFrame::OnMenuFileSave(wxCommandEvent &event) {
	...
	if (SaveDialog->ShowModal() == wxID_OK) {
		m_pTextCtrl->SaveFile(SaveDialog->GetPath()) ? SetStatusText(_T("Saved")) : SetStatusText(_T("Save Failed"));
	}
	SaveDialog->Close();
}
