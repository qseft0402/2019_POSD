...
TEST(ellipse, area)
{
  ...
  Ellipse ell2 (1, 1);
  ASSERT_NEAR(3.14, ell2.area(), 0.1);
}
...
TEST(ellipse, perimeter)
{
  ...
  Shape * ell2 = new Ellipse(1,1);
  ASSERT_NEAR(6.28, ell2->perimeter(), 0.1);
}

TEST(ComplexShapes, Perimeter)
{
    Shape *ell = new Ellipse(1,1); //6.28
    Shape * rectangle = new Rectangle(2,4); //12
...
{
    Shape * circle = new Circle(1); // 3.14
    Shape * circle2 = new Circle(2); //
    Shape * rectangle = new Rectangle(2,4); // 8
    Shape * ell = new Ellipse(1,1); // 6.28
...
{
    Shape * circle = new Circle(1); // 3.14
    Shape * circle2 = new Circle(2); // 12...
    Shape * rectangle = new Rectangle(2,4); // 8
    Shape * ell = new Ellipse(1,1); // 3.14

    std::vector<Shape *> shapes = {circle,rectangle,circle2};
    ComplexShapes * complexShapes2 = new ComplexShapes(&shapes);

    std::vector<Shape *> shapes2 = {ell,complexShapes2, complexShapes2};
    ...
    complexShapes->accept(fv);
    ASSERT_EQ(6,fv->findResult().size()); // circle2 rectangle complexShapes2
}
...
TEST(Visitor, VisitDifferentComposite)
{
    ...
    complexShapes2->add(complexShapes); // ell complexShapes
    complexShapes2->accept(fv);
    ASSERT_EQ(1,fv->findResult().size());
}
...
TEST(Visitor, TRUEE)
{
    ...
    std::vector<Shape *> shapes = {circle,rectangle,circle2};
    ComplexShapes * complexShapes = new ComplexShapes(&shapes);
    std::vector<Shape *> shapes2 = {ell, circle, complexShapes};
    ComplexShapes * complexShapes2 = new ComplexShapes(&shapes2);
