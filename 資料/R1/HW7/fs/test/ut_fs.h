...
TEST (FindNodeByPathnameVisitor, FileInNestedFolder) {
    ...
    Node* folderFile = new File("./test_data/folder/folder/folder");
    Node* b_out = new File("./test_data/folder/folder/b.out");
    folder2->add(b_out);
    folder2->add(folderFile);
    folder->add(folder2);
    test_data->add(folder);
    std::vector<std::string>* names = new std::vector<std::string>{std::string("test_data"), std::string("folder"), std::string("folder"), std::string("b.out")};
    FindNodeByPathnameVisitor* fv = new FindNodeByPathnameVisitor(names);
    test_data->accept(fv);
    //cout << "123: " << fv->getNode()->name() << endl;
    ASSERT_EQ(b_out, fv->getNode()); //->getParent()->getParent()->getParent()
}/*
TEST (FindTest, File)
{
  Node * hello = new Link("./test_data/hello");
